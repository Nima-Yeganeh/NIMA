...
# Interface name
interface=$({ ip link show eth0 &>/dev/null && echo "eth0"; } || { ip link show enp1s0 &>/dev/null && echo "enp1s0"; } || { ip link show ens3 &>/dev/null && echo "ens3"; } || { ip link show enp3s0 &>/dev/null && echo "enp3s0"; })
