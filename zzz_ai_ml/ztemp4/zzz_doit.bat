...
C:\ProgramData\chocolatey\bin\ffmpeg -i "%input_file%" -vn -codec:a libmp3lame -b:a 320k -ar 44100 -ac 2 "%mp3_output_file%"
...
C:\ProgramData\chocolatey\bin\ffmpeg -i "%video_file%" -an -c:v copy "%output_tempfile%"
C:\ProgramData\chocolatey\bin\ffmpeg -i "%output_tempfile%" -i "%audio_file%" -c:v copy -c:a aac -b:a 192k -ac 2 -shortest "%output_file%"
