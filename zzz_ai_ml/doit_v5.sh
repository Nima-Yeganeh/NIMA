...
python -m pytgpt generate "I have a course about $zmainidea $zkeywords - give me $numberoftopics topics and domains without explanation for students to learn in this course" > zcrsdomains.txt
sleep $sleeptime
...
echo >> zzz.txt
echo >> zzz.txt
crstitle=$(python -m pytgpt generate "give me a course title min 50 and max 60 characters about $zmainidea $zkeywords" | sed 's/"//g' | sed 's/\// /g' | sed 's/:/ /g')
echo $crstitle
# sleep $sleeptime
crssubtitle=$(python -m pytgpt generate "give me a course subtitle min 110 to max 120 characters about $zmainidea $zkeywords" | sed 's/"//g' | sed 's/\// /g' | sed 's/:/ /g')
echo $crssubtitle
# sleep $sleeptime
